...
#include <lvgl.h>
#include "brightness_test_screen.h"

LOG_MODULE_REGISTER(brightness_test_screen, LOG_LEVEL_INF);
...
{
    LOG_DBG("Initializing brightness test screen");

    // Create brightness test screen
    brightness_screen = lv_obj_create(NULL);
    lv_obj_set_style_bg_color(brightness_screen, lv_color_hex(0x000000), LV_PART_MAIN);

#ifdef CONFIG_BOARD_NATIVE_SIM
    // Add circle border for native sim
    lv_obj_t *circle_border = lv_obj_create(brightness_screen);
    ...
    title_label = lv_label_create(brightness_screen);
    ...
    instruction_label = lv_label_create(brightness_screen);
    ...
    timeout_label = lv_label_create(brightness_screen);
...
{
    lv_label_set_text(instruction_label, "Press ANY button to\nstart backlight test");
    lv_obj_set_style_text_color(instruction_label, lv_color_hex(0x00AAFF), LV_PART_MAIN);
    lv_label_set_text(timeout_label, "");
    lv_obj_set_style_text_color(timeout_label, lv_color_hex(0xCCCCCC), LV_PART_MAIN);
...
{
    lv_label_set_text(instruction_label, "Cycling brightness\nDIM <-> BRIGHT");
    lv_obj_set_style_text_color(instruction_label, lv_color_hex(0xFFFF00), LV_PART_MAIN);
}

void brightness_test_screen_ask_confirmation(void)
{
    lv_label_set_text(instruction_label, "Did you see brightness\nchange? Press ANY button if YES");
    lv_obj_set_style_text_color(instruction_label, lv_color_hex(0x00FF00), LV_PART_MAIN);
}
