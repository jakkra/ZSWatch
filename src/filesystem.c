...
FS_LITTLEFS_DECLARE_DEFAULT_CONFIG(littlefs);
static struct fs_mount_t lfs_storage_mnt = {
    .type = FS_LITTLEFS,
    .fs_data = &littlefs,
    .storage_dev = (void *)FLASH_AREA_ID(littlefs),
    .mnt_point = "/lfs",
};
...
static void incr_pattern(uint8_t *p, uint16_t size)
{
    int not_filled_idx = 0;
...
static int littlefs_flash_erase(unsigned int id, bool erase)
{
...
static int littlefs_mount(struct fs_mount_t *mp)
{
    ...
    rc = littlefs_flash_erase((uintptr_t)mp->storage_dev, false);
    if (rc < 0) {
        return rc;
    }
...
void filesystem_test(void)
{
    ...
    snprintf(fname2, sizeof(fname2), "%s/watchface.bin", mp->mnt_point);
