        ...
        case TIMER_EVT_START_PAUSE_RESUME:
        {
            ...
            if (timers[timer_id].state == TIMER_STATE_STOPPED || timers[timer_id].state == TIMER_STATE_PAUSED) {
                ...
                if (timers[timer_id].type == TYPE_ALARM) {
                    ...
                    zsw_timer_id = zsw_alarm_add_timer(timers[timer_id].remaining_hour, timers[timer_id].remaining_min, timers[timer_id].remaining_sec, alarm_triggered_cb, (void*)timer_id);
                }
                if (zsw_timer_id < 0) {
